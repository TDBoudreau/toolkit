...
// UploadFiles uploads one or more files to a specified directory, and the files a random name. It returns a slice
// containing the newly named files, the original file names, the size of the file, and potentially an error.
// If the optional last parameter is set to false, then we will not rename the files and use the original file names.
	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				allowed := false // flag, assume the file type is not allowed
...
func (t *Tools) Slugify(s string) (string, error) {
	...
	re := regexp.MustCompile(`[^a-z\d]+`) // any letter a-z and any digits, any length
...
// The final parameter, client, is optional. If no client is specified, we use the standard http.Client.
func (t *Tools) PushJSONToRemote(uri string, data any, client ...*http.Client) (*http.Response, int, error) {
	...
	// return response, status code, and error (if any)
